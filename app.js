    ...
    bookList.forEach((book, index) => {
        bookListHtml += `<div class="col-5 ${index %2 == 0 && "offset-2"} my-5">
          <div class="row book_card">
            <div class="col-6">
              <img
                class="img-fluid shadow book-img"
                src="${book.imgSource}"
                width="258"
                height="400"
                onclick="big_open(this.src)"
              >
            </div>
            <div class="col-6 d-flex flex-column justify-content-between">
              ...
              <div>
                <span class="black fw-bold fs-4 me-2">${book.price.toFixed(2)}₺</span>
                ${book.oldPrice ? `<span class="fs-4 fw-bold old_price">${book.oldPrice.toFixed(2)}₺</span>` :""}
              </div>
...
const listBasketItems = () => {
  ...
  basketList.forEach(item => {
    ...
    basketListHtml += `<li class="basket_item">
            <img src="${item.product.imgSource}" width="100" height="100" class="book-img">
            <div class="basket_item-info">
              <h3 class="book_name">${item.product.name}</h3>
              <span class="book_price">${item.product.price.toFixed(2)}₺</span><br />
...
document.addEventListener("click", function (event) {
    ...
    const tiklananEleman = event.target;
    if (tiklananEleman.classList.contains("book-img")) {

        const lightboxDiv = document.getElementById("lightbox");
        const buyukResim = document.getElementById("lightbox-img");

        lightboxDiv.classList.add("active");
        buyukResim.src = tiklananEleman.src;
    }
});
...
if (searchIcon != null && searchInput != null) {
  ...
  searchIcon.addEventListener("click", function () {
    ...
    const aranan = searchInput.value.toLowerCase().trim();
    if (aranan === "") {
      createBookItemsHtml();
      return;
    }
    ...
    for (let i = 0; i < bookList.length; i++) {
      if (bookList[i].name.toLowerCase() === aranan) {
        bulunanlar.push(bookList[i]);
      }
    }
